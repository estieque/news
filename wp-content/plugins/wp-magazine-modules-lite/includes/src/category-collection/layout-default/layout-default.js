/**
 * Post Category Co0llection block layout default - editor preview
 */
...
class CategoryCollectionDefault extends Component {
    constructor( props ) {
        super(...arguments)
    }
...
export default withSelect( ( select, props ) => {
    ...
    blockCategories.map( ( catid ) => {
        ...
        let catContent = getEntityRecords( 'taxonomy', 'category', catQuery )
        if( Array.isArray(catContent) && catContent.length ) {
            singlecatContent.cat_id = catContent[0].id
            singlecatContent.cat_name = catContent[0].name
            singlecatContent.cat_desc = catContent[0].description
            singlecatContent.cat_link = catContent[0].link
            singlecatContent.cat_count = catContent[0].count
        }
        const PostsQuery = {
            per_page: 1,
            categories: catid
